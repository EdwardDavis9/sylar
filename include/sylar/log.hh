...
/**
 * @class 日志事件, 定义了一些和日志属性相关的内容信息. 日志的原子信息
 */
class LogEvent {
  public:
  ...
  private:
    ...
    uint64_t m_time;               /**< 时间戳 */
    std::stringstream m_ss;        /**< 实际传递的日志信息 */
...
class LogAppender {
  ...
  protected:
    ...
    bool m_hasFormatter = false;   /**< 是否存在基本格式 */
    MutexType m_mutex;             /**< 系统层面上自旋锁 */
    LogFormatter::ptr m_formatter; /**< 日志输出器所属的格式器 */
};

/**
 * @brief   Logger 日志器类
 * @details 管理日志级别, 输出器以及格式解析器, 是日志系统的核心类
 */
class Logger : public std::enable_shared_from_this<Logger> {
  ...
  public:
    ...
    int getAppenderSize() { return m_appenders.size(); }

    /**
     * @brief     设置日志器的 formatter
     * @details   给不存在 formatter 的 appender 设置一个默认的 formatter
     * @param[in] var 待设置的 formatter
     */
...
class LoggerManager {
  ...
  private:
    std::map<std::string, Logger::ptr> m_loggers; // 日志器集
    Logger::ptr m_root;
    MutexType m_mutex;
};
