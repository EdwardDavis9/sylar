...
void test()
{
        ...
        {                                                                    \
            ...
            for (int i = 0; i < len; ++i) {                                  \
                if (randNum == 1) {                                          \
                    ...
                    vec.push_back((static_cast<float>(rand())                \
    ...
    #define XX(type, len, write_fun, read_fun, node_size)                      \
        {                                                                      \
            ...
            SYLAR_ASSERT(ba2->readFromFile("/tmp/" #type "_" #len              \
                                           "-" #read_fun ".dat"));             \
            ba2->setPosition(0);                                               \
...
void testFloatBasic()
{
    ...
    std::cout << "验证浮点数的写入和读取的数据精度相关内容." << std::endl;
    sylar::ByteArray ba(4096);

    // 测试一些常见值
    std::vector<float> test_floats = {0.0f,      1.0f,     -1.0f,     3.14159f,
                                      -2.71828f, 123.456f, -987.654f, 1.0e10f,
                                      -1.0e10f,  1.0e-10f, -1.0e-10f};
    ...
    for (auto expected : test_floats) {
        ...
        if (rel_error > 1e-6f) {
            SYLAR_LOG_ERROR(g_logger)
                << "Float mismatch: expected " << expected << ", got " << actual
                << ", diff = " << diff;
        }
        else {
...
{
    std::cout << "测试浮点数的表示范围." << std::endl;
    sylar::ByteArray ba(4096);
    // 浮点数边界值
    std::vector<float> boundaries = {
        std::numeric_limits<float>::min(),       // 最小浮点正正规数
        std::numeric_limits<float>::max(),       // 最大浮点正数
        -std::numeric_limits<float>::max(),      // 最小浮点负数
    ...
    for (size_t i = 0; i < boundaries.size(); ++i) {
        ...
        if (std::isnan(expected)) {
            // NaN 需要特殊处理
            if (!std::isnan(actual)) {
                SYLAR_LOG_ERROR(g_logger) << "ERROR: NaN not preserved";
            }
            SYLAR_LOG_INFO(g_logger) << actual;
        }
            ...
            if (actual != expected) {
                SYLAR_LOG_ERROR(g_logger)
                    << "ERROR: Infinity mismatch: expected " << expected
                    << ", got " << actual;
            }
            SYLAR_LOG_INFO(g_logger) << actual;
        }
            ...
            if (rel_error > 1e-6f) {
                SYLAR_LOG_ERROR(g_logger)
                    << "ERROR: Float boundary mismatch: expected " << expected
                    << ", got " << actual;
            }
            SYLAR_LOG_INFO(g_logger) << actual;
        }
...
void testFloatRandom()
{
    ...
    for (int i = 0; i < COUNT; ++i) {
        ...
        // 允许1个ULP的误差(考虑到可能的舍入)
        if (e_bits != a_bits
            && std::abs(static_cast<int32_t>(e_bits - a_bits)) > 1)
        {
            ++errors;
            if (errors < 10) { // 只打印前10个错误
                SYLAR_LOG_WARN(g_logger)
                    << "Float mismatch at " << i << ": expected " << expected
                    << " (0x" << std::hex << e_bits << ")"
                    << ", got " << actual << " (0x" << a_bits << ")";
            }
        }
    }

    SYLAR_LOG_INFO(g_logger)
        << "Random float test: " << COUNT << " values, " << errors << " errors";
}
...
void testDouble()
{
    ...
    for (auto expected : test_doubles) {
        ...
        if (std::isnan(expected)) {
            ...
            if (actual != expected) {
                SYLAR_LOG_ERROR(g_logger)
                    << "Double Infinity mismatch: " << expected << " vs "
                    << actual;
            }
        }
        else {
...
void testFloatEndian()
{
    ...
    SYLAR_LOG_INFO(g_logger)
        << "Native bytes: " << std::hex << (int)native_bytes[0] << " "
        << std::hex << (int)native_bytes[1] << " " << std::hex
        << (int)native_bytes[2] << " " << std::hex << (int)native_bytes[3];

    SYLAR_LOG_INFO(g_logger)
        << "Big endian bytes: " << std::hex << (int)bytes_big[0] << " "
        << std::hex << (int)bytes_big[1] << " " << std::hex << (int)bytes_big[2]
        << " " << std::hex << (int)bytes_big[3];

    SYLAR_LOG_INFO(g_logger)
        << "Little endian bytes: " << std::hex << (int)bytes_little[0] << " "
        << std::hex << (int)bytes_little[1] << " " << std::hex
        << (int)bytes_little[2] << " " << std::hex << (int)bytes_little[3];
}
